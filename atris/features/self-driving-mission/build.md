...
- Generated prompts: navigator step 3 and executor steps 1 and 5, plus the `--execute` executor prompt, now name `atris task add/plan/delegate/claim/ready` and call `atris/TODO.md` a generated view. `atris task accept` is never suggested to an agent. Handoffs load `atris task show <exact-task-id> --json` before claim/edit, retain raw instructions and the task owner, carry the expected owner and mission, reload and validate the live assignment before enabling backend edits, refuse missing or mismatched dispatches, and recognize existing user authorization without changing approval gates.
