...
test('billed auth uses leftover profile agent_token scopes when env repeats that leftover', async t => {
  sandbox(t);
  const leftover = 'fresh-profile-agent-token';
  const expiresAt = new Date(Date.now() + 60_000).toISOString();
  ...
  process.env.ATRIS_PROFILE = 'owner';
  process.env.ATRIS_TOKEN = leftover;
  ...
  const result = await ensureBilledCommandAuth('youtube', {
    ...
    apiRequestJson: async () => {
      throw new Error('env-repeated profile leftover must not remint');
    },
  });
  assert.equal(result.ok, true);
  assert.equal(result.minted, false);
...
test('billed auth does not remint youtube from an env-repeated x-search-only profile leftover', async t => {
  sandbox(t);
  const leftover = 'fresh-profile-agent-token';
  auth.saveProfile('owner', {
    token: 'stored-user-jwt',
